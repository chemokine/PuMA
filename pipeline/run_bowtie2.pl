...
croak("Error: PuMA stanza does not exist in conf file\n") if(($Conf->get('Bowtie2','bowtie2')) eq -1);
my $program = $Conf->get('Bowtie2','bowtie2');
my $program_options = $Conf->get('Bowtie2','bowtie2_options');
my $bt2db = $Conf->get('Bowtie2','bowtie2_db');
...
#########################################################
  ...
  # Build Command:
  $program_options .= " --quiet -x $bt2db -U $file --al $run_id.bowtie2.host --un $run_id.bowtie2.out";
...
sub var_error(){
  print "\n  Description:\n";
  print "  run_bowtie2.pl processes the input file given by the -f option and processess it through bowtie2\n";
  print "  Usage:\n";
  print "  run_bowtie2.pl -f <inputfile> -o <outputfile> -x <bowtie2 db> -r <run_id> -c <run config file>\n";
  print "\n\n\n";
